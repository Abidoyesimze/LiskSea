...
export const SwapPanel = () => {
  ...
  const { data: tokenAAddress, error: tokenAError } = useScaffoldContractRead({
    contractName: "SimpleDEX",
    functionName: "tokenA",
  });

  const { data: tokenBAddress, error: tokenBError } = useScaffoldContractRead({
    contractName: "SimpleDEX",
    functionName: "tokenB",
  });
  ...
  // SimpleDEX contract address (hardcoded from deployment)
  const dexAddress = "0x279DBdCD8aA85Bc40E1f834B1ee3ABb447A81b04";
  ...
  // Get swap quote
  const tokenInAddress = isTokenAInput ? tokenAAddress : tokenBAddress;
  const amountIn = inputAmount ? parseUnits(inputAmount, isTokenAInput ? 18 : 6) : 0n;

  const { data: swapQuote, error: swapQuoteError } = useScaffoldContractRead({
    contractName: "SimpleDEX",
